...
- 判别对象的位置
    ...
    - 对象外部
    - 子类中
- python中下划线使用（http://blog.csdn.net/handsomekang/article/details/40301）
- 私有
...
- super
    ...
    - super于父类直接没有任何实质性关系，但通过super可以调用父类
...
- 菱形继承/砖石继承问题
    - 多个子类继承自同一个父类，这些子类又被同一个类继承，于是继承关系图形
    形成一个菱形图谱
...
- 操作类
    - '__init__':构造函数 
    - '__new__':对象实例化方法（实例化时第一个被调用，之后是__init__）
        此函数较特殊，一般不需要使用
    - '__call__': 对象当函数使用的时候触发
    - '__str__': 当对象被当作字符串使用的时候调用
    - '__repr__': 返回字符串，与'__str__'的具体区别请百度 
- 描述符相关(百度下了解)
    - '__set__':
    - '__get__':
    - '__delete__':
